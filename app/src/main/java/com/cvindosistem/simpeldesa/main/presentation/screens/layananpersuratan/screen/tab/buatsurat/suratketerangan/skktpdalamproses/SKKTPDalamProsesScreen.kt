...
import com.cvindosistem.simpeldesa.main.presentation.screens.layananpersuratan.viewmodel.suratketerangan.ktpdalamproses.SKKTPDalamProsesViewModel.SKKTPDalamProsesEvent
import kotlinx.coroutines.delay

@OptIn(ExperimentalAnimationApi::class)
@Composable
fun SKKTPDalamProsesScreen(
    skKTPDalamProsesViewModel: SKKTPDalamProsesViewModel,
    navController: NavController
) {
    val currentStep by remember { derivedStateOf { skKTPDalamProsesViewModel.currentStep } }
    val showConfirmationDialog by remember { derivedStateOf { skKTPDalamProsesViewModel.showConfirmationDialog } }
    val showPreviewDialog by remember { derivedStateOf { skKTPDalamProsesViewModel.showPreviewDialog } }
    val isLoading by remember { derivedStateOf { skKTPDalamProsesViewModel.isLoadingUserData } }
    val hasFormData by remember { derivedStateOf { skKTPDalamProsesViewModel.hasFormData() } }
    val totalSteps = 2
    ...
    LaunchedEffect(skKTPDalamProsesViewModel.skKTPDalamProsesEvent) {
        skKTPDalamProsesViewModel.skKTPDalamProsesEvent.collect { event ->
            when (event) {
                is SKKTPDalamProsesEvent.SubmitSuccess -> {
                ...
                is SKKTPDalamProsesEvent.SubmitError -> {
                    errorDialogTitle = "Gagal Mengirim"
                    errorDialogMessage = event.message
                    showErrorDialog = true
                }
                is SKKTPDalamProsesEvent.UserDataLoadError -> {
                    errorDialogTitle = "Gagal Memuat Data"
                    errorDialogMessage = event.message
                    showErrorDialog = true
                }
                is SKKTPDalamProsesEvent.ValidationError -> {
                    snackbarHostState.showSnackbar(
                        message = "Mohon lengkapi semua field yang diperlukan",
                        duration = SnackbarDuration.Short
                    )
                }
                is SKKTPDalamProsesEvent.StepChanged -> {
    ...
    Scaffold(
        ...
        bottomBar = {
            AppBottomBar(
                onPreviewClick = {
                    skKTPDalamProsesViewModel.showPreview()
                },
                onBackClick = if (currentStep > 1) {
                    { skKTPDalamProsesViewModel.previousStep() }
                } else null,
                onContinueClick = if (currentStep < totalSteps) {
                    { skKTPDalamProsesViewModel.nextStep() }
                } else null,
                onSubmitClick = if (currentStep == totalSteps) {
                    { skKTPDalamProsesViewModel.showConfirmationDialog() }
                } else null
            )
        }
    ) { paddingValues ->
        Box(modifier = Modifier.padding(paddingValues)) {
            AppStepAnimatedContent(
                ...
                when (step) {
                    1 -> SKKTPDalamProses1Content(
                        viewModel = skKTPDalamProsesViewModel,
                        modifier = Modifier.fillMaxSize()
                    )
                    2 -> SKKTPDalamProses2Content(
                        viewModel = skKTPDalamProsesViewModel,
                        modifier = Modifier.fillMaxSize()
                    )
                }
            ...
            if (showPreviewDialog) {
                PreviewDialog(
                    viewModel = skKTPDalamProsesViewModel,
                    onDismiss = {
                        skKTPDalamProsesViewModel.dismissPreview()
                    },
                    onSubmit = {
                        skKTPDalamProsesViewModel.dismissPreview()
                        skKTPDalamProsesViewModel.showConfirmationDialog()
            ...
            if (showConfirmationDialog) {
                SubmitConfirmationDialog(
                    onConfirm = {
                        skKTPDalamProsesViewModel.confirmSubmit()
                    },
                    onDismiss = {
                        skKTPDalamProsesViewModel.dismissConfirmationDialog()
                    },
                    onPreview = {
                        skKTPDalamProsesViewModel.dismissConfirmationDialog()
                        skKTPDalamProsesViewModel.showPreview()
            ...
            if (showErrorDialog) {
                ErrorDialog(
                    ...
                    onDismiss = {
                        ...
                        skKTPDalamProsesViewModel.clearError()
                    }
                )
            }

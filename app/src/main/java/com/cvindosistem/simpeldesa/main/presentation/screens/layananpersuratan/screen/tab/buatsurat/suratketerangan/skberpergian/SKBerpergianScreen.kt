...
import com.cvindosistem.simpeldesa.main.presentation.screens.layananpersuratan.screen.tab.buatsurat.suratketerangan.skberpergian.SKBerpergian1Content
import com.cvindosistem.simpeldesa.main.presentation.screens.layananpersuratan.screen.tab.buatsurat.suratketerangan.skberpergian.SKBerpergian2Content
import com.cvindosistem.simpeldesa.main.presentation.screens.layananpersuratan.screen.tab.buatsurat.suratketerangan.skberpergian.SKBerpergian3Content
...
fun SKBerpergianScreen(
    ...
    val currentStep by remember { derivedStateOf { skBerpergianViewModel.currentStep } }
    val showConfirmationDialog by remember { derivedStateOf { skBerpergianViewModel.showConfirmationDialog } }
    val showPreviewDialog by remember { derivedStateOf { skBerpergianViewModel.showPreviewDialog } }
    val isLoading by remember { derivedStateOf { skBerpergianViewModel.isLoading } }
    val hasFormData by remember { derivedStateOf { skBerpergianViewModel.hasFormData() } }
    val totalSteps = 3
    ...
    LaunchedEffect(skBerpergianViewModel.skBerpergianEvent) {
        skBerpergianViewModel.skBerpergianEvent.collect { event ->
            when (event) {
                is SKBerpergianViewModel.SKBerpergianEvent.SubmitSuccess -> {
                ...
                is SKBerpergianViewModel.SKBerpergianEvent.SubmitError -> {
                    errorDialogTitle = "Gagal Mengirim"
                    errorDialogMessage = event.message
                    showErrorDialog = true
                }
                is SKBerpergianViewModel.SKBerpergianEvent.UserDataLoadError -> {
                    errorDialogTitle = "Gagal Memuat Data"
                    errorDialogMessage = event.message
                    showErrorDialog = true
                }
                is SKBerpergianViewModel.SKBerpergianEvent.ValidationError -> {
                    snackbarHostState.showSnackbar(
                        message = "Mohon lengkapi semua field yang diperlukan",
                        duration = SnackbarDuration.Short
                    )
                }
                is SKBerpergianViewModel.SKBerpergianEvent.StepChanged -> {
    ...
    Scaffold(
        ...
        bottomBar = {
            AppBottomBar(
                onPreviewClick = {
                    skBerpergianViewModel.showPreview()
                },
                onBackClick = if (currentStep > 1) {
                    { skBerpergianViewModel.previousStep() }
                } else null,
                onContinueClick = if (currentStep < totalSteps) {
                    { skBerpergianViewModel.nextStep() }
                } else null,
                onSubmitClick = if (currentStep == totalSteps) {
                    { skBerpergianViewModel.showConfirmationDialog() }
                } else null
            )
        }
    ) { paddingValues ->
        Box(modifier = Modifier.padding(paddingValues)) {
            ...
            if (showPreviewDialog) {
                PreviewDialog(
                    viewModel = skBerpergianViewModel,
                    onDismiss = {
                        skBerpergianViewModel.dismissPreview()
                    },
                    onSubmit = {
                        skBerpergianViewModel.dismissPreview()
                        skBerpergianViewModel.showConfirmationDialog()
            ...
            if (showConfirmationDialog) {
                SubmitConfirmationDialog(
                    onConfirm = {
                        skBerpergianViewModel.confirmSubmit()
                    },
                    onDismiss = {
                        skBerpergianViewModel.dismissConfirmationDialog()
                    },
                    onPreview = {
                        skBerpergianViewModel.dismissConfirmationDialog()
                        skBerpergianViewModel.showPreview()
            ...
            if (showErrorDialog) {
                ErrorDialog(
                    ...
                    onDismiss = {
                        ...
                        skBerpergianViewModel.clearError()
                    }
                )
            }
...
private fun PreviewDialog(
    ...
    BaseDialog(
        ...
        LazyColumn(
            ...
            item {
                PreviewSection(
                    title = "Informasi Kepergian",
                    content = {
                        PreviewItem("Tempat Tujuan", viewModel.tempatTujuanValue)
                        PreviewItem("Maksud Tujuan", viewModel.maksudTujuanValue)
                        PreviewItem("Tanggal Keberangkatan", viewModel.tanggalKeberangkatanValue)
                        PreviewItem(
                            "Lama Kepergian",
                            "${viewModel.lamaValue} ${viewModel.satuanLamaValue}".trim()
                        )
                        PreviewItem("Jumlah Pengikut", viewModel.jumlahPengikutValue)
                    }
                )
            }

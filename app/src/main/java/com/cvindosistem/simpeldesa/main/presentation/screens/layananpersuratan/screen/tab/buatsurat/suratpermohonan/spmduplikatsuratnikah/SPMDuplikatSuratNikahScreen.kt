...
@Composable
fun SPMDuplikatSuratNikahScreen(
    spmDuplikatSuratNikahViewModel: SPMDuplikatSuratNikahViewModel,
    navController: NavController
) {
    val currentStep by remember { derivedStateOf { spmDuplikatSuratNikahViewModel.currentStep } }
    val showConfirmationDialog by remember { derivedStateOf { spmDuplikatSuratNikahViewModel.showConfirmationDialog } }
    ...
    LaunchedEffect(spmDuplikatSuratNikahViewModel.spmDuplikatSuratNikahEvent) {
        spmDuplikatSuratNikahViewModel.spmDuplikatSuratNikahEvent.collect { event ->
            when (event) {
                is SPMDuplikatSuratNikahViewModel.SPMDuplikatSuratNikahEvent.SubmitSuccess -> {
                ...
                is SPMDuplikatSuratNikahViewModel.SPMDuplikatSuratNikahEvent.SubmitError -> {
                    errorDialogTitle = "Gagal Mengirim"
                    errorDialogMessage = event.message
                    showErrorDialog = true
                }
                is SPMDuplikatSuratNikahViewModel.SPMDuplikatSuratNikahEvent.UserDataLoadError -> {
                    errorDialogTitle = "Gagal Memuat Data"
                    errorDialogMessage = event.message
                    showErrorDialog = true
                }
                is SPMDuplikatSuratNikahViewModel.SPMDuplikatSuratNikahEvent.ValidationError -> {
                    snackbarHostState.showSnackbar(
                        message = "Mohon lengkapi semua field yang diperlukan",
                        duration = SnackbarDuration.Short
                    )
                }
                is SPMDuplikatSuratNikahViewModel.SPMDuplikatSuratNikahEvent.StepChanged -> {
    ...
    Scaffold(
        ...
        bottomBar = {
            AppBottomBar(
                onPreviewClick = {
                    spmDuplikatSuratNikahViewModel.showPreview()
                },
                onBackClick = if (currentStep > 1) {
                    { spmDuplikatSuratNikahViewModel.previousStep() }
                } else null,
                onContinueClick = if (currentStep < totalSteps) {
                    { spmDuplikatSuratNikahViewModel.nextStep() }
                } else null,
                onSubmitClick = if (currentStep == totalSteps) {
                    { spmDuplikatSuratNikahViewModel.showConfirmationDialog() }
                } else null
            )
        }
    ) { paddingValues ->
        Box(modifier = Modifier.padding(paddingValues)) {
            AppStepAnimatedContent(
                ...
                when (step) {
                    1 -> SPMDuplikatSuratNikah1Content(
                        viewModel = spmDuplikatSuratNikahViewModel,
                        modifier = Modifier.fillMaxSize()
                    )
                    2 -> SPMDuplikatSuratNikah2Content(
                        viewModel = spmDuplikatSuratNikahViewModel,
                        modifier = Modifier.fillMaxSize()
                    )
            ...
            if (showPreviewDialog) {
                PreviewDialog(
                    viewModel = spmDuplikatSuratNikahViewModel,
                    onDismiss = {
                        spmDuplikatSuratNikahViewModel.dismissPreview()
                    },
                    onSubmit = {
                        spmDuplikatSuratNikahViewModel.dismissPreview()
                        spmDuplikatSuratNikahViewModel.showConfirmationDialog()
            ...
            if (showConfirmationDialog) {
                SubmitConfirmationDialog(
                    onConfirm = {
                        spmDuplikatSuratNikahViewModel.confirmSubmit()
                    },
                    onDismiss = {
                        spmDuplikatSuratNikahViewModel.dismissConfirmationDialog()
                    },
                    onPreview = {
                        spmDuplikatSuratNikahViewModel.dismissConfirmationDialog()
                        spmDuplikatSuratNikahViewModel.showPreview()
            ...
            if (showErrorDialog) {
                ErrorDialog(
                    ...
                    onDismiss = {
                        ...
                        spmDuplikatSuratNikahViewModel.clearError()
                    }
                )
            }
            ...
            item {
                PreviewSection(
                    ...
                    content = {
                        ...
                        PreviewItem("No. Kartu Keluarga", viewModel.noKkValue)
                        PreviewItem("Pekerjaan", viewModel.pekerjaanValue)
                        PreviewItem("Pendidikan ID", viewModel.pendidikanIdValue)
                        PreviewItem("Agama ID", viewModel.agamaIdValue)
                        PreviewItem("Alamat", viewModel.alamatValue)
                    }
                )
            }

            // Step 2: Informasi Pernikahan
            item {
                PreviewSection(
                    title = "Informasi Pernikahan",
                    content = {
                        PreviewItem("Nama Pasangan", viewModel.namaPasanganValue)
                        PreviewItem("Tanggal Nikah", viewModel.tanggalNikahValue)
                        PreviewItem("Kepala Keluarga", viewModel.kepalaKeluargaValue)
                        PreviewItem("Kecamatan KUA", viewModel.kecamatanKuaValue)
                    }
                )
            }

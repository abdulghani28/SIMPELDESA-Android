...
@Composable
fun SKWaliHakimScreen(
    skWaliHakimViewModel: SKWaliHakimViewModel,
    navController: NavController
) {
    val currentStep by remember { derivedStateOf { skWaliHakimViewModel.currentStep } }
    val showConfirmationDialog by remember { derivedStateOf { skWaliHakimViewModel.showConfirmationDialog } }
    val showPreviewDialog by remember { derivedStateOf { skWaliHakimViewModel.showPreviewDialog } }
    val isLoading by remember { derivedStateOf { skWaliHakimViewModel.isLoading } }
    val hasFormData by remember { derivedStateOf { skWaliHakimViewModel.hasFormData() } }
    val totalSteps = 2
    ...
    LaunchedEffect(skWaliHakimViewModel.skWaliHakimEvent) {
        skWaliHakimViewModel.skWaliHakimEvent.collect { event ->
            when (event) {
                is SKWaliHakimViewModel.SKWaliHakimEvent.SubmitSuccess -> {
                ...
                is SKWaliHakimViewModel.SKWaliHakimEvent.SubmitError -> {
                    errorDialogTitle = "Gagal Mengirim"
                    errorDialogMessage = event.message
                    showErrorDialog = true
                }
                is SKWaliHakimViewModel.SKWaliHakimEvent.UserDataLoadError -> {
                    errorDialogTitle = "Gagal Memuat Data"
                    errorDialogMessage = event.message
                    showErrorDialog = true
                }
                is SKWaliHakimViewModel.SKWaliHakimEvent.ValidationError -> {
                    snackbarHostState.showSnackbar(
                        message = "Mohon lengkapi semua field yang diperlukan",
                        duration = SnackbarDuration.Short
                    )
                }
                is SKWaliHakimViewModel.SKWaliHakimEvent.StepChanged -> {
    ...
    Scaffold(
        ...
        bottomBar = {
            AppBottomBar(
                onPreviewClick = {
                    skWaliHakimViewModel.showPreview()
                },
                onBackClick = if (currentStep > 1) {
                    { skWaliHakimViewModel.previousStep() }
                } else null,
                onContinueClick = if (currentStep < totalSteps) {
                    { skWaliHakimViewModel.nextStep() }
                } else null,
                onSubmitClick = if (currentStep == totalSteps) {
                    { skWaliHakimViewModel.showConfirmationDialog() }
                } else null
            )
        }
    ) { paddingValues ->
        Box(modifier = Modifier.padding(paddingValues)) {
            AppStepAnimatedContent(
                ...
                when (step) {
                    1 -> SKWaliHakim1Content(
                        viewModel = skWaliHakimViewModel,
                        modifier = Modifier.fillMaxSize()
                    )
                    2 -> SKWaliHakim2Content(
                        viewModel = skWaliHakimViewModel,
                        modifier = Modifier.fillMaxSize()
                    )
                }
            ...
            if (showPreviewDialog) {
                PreviewDialog(
                    viewModel = skWaliHakimViewModel,
                    onDismiss = {
                        skWaliHakimViewModel.dismissPreview()
                    },
                    onSubmit = {
                        skWaliHakimViewModel.dismissPreview()
                        skWaliHakimViewModel.showConfirmationDialog()
            ...
            if (showConfirmationDialog) {
                SubmitConfirmationDialog(
                    onConfirm = {
                        skWaliHakimViewModel.confirmSubmit()
                    },
                    onDismiss = {
                        skWaliHakimViewModel.dismissConfirmationDialog()
                    },
                    onPreview = {
                        skWaliHakimViewModel.dismissConfirmationDialog()
                        skWaliHakimViewModel.showPreview()
            ...
            if (showErrorDialog) {
                ErrorDialog(
                    ...
                    onDismiss = {
                        ...
                        skWaliHakimViewModel.clearError()
                    }
                )
            }

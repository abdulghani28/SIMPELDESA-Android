...
class SKNikahWargaNonMuslimDataLoader(
    ...
    private val validator: SKNikahWargaNonMuslimValidator
) {
    ...
    suspend fun loadAllInitialData(): Result<Unit> {
        return try {
            ...
            if (failedResults.isNotEmpty()) {
                val errors = failedResults.mapNotNull {
                    it.exceptionOrNull()?.message
                }.joinToString(", ")
                stateManager.updateErrorMessage("Gagal memuat data: $errors")
                Result.failure(Exception("Gagal memuat data: $errors"))
            } else {
                Result.success(Unit)
            }
        } catch (e: Exception) {
            val errorMsg = e.message ?: "Gagal memuat data awal"
            stateManager.updateErrorMessage(errorMsg)
            Result.failure(Exception(errorMsg))
        } finally {
    ...
    suspend fun populateUserDataForSuami(): Result<Unit> {
        return try {
            when (val result = getUserInfoUseCase()) {
                is UserInfoResult.Success -> {
                    val userData = result.data.data
                    stateManager.populateUserData(userData)
                    validator.clearSuamiFieldErrors()
                    Result.success(Unit)
                }
                is UserInfoResult.Error -> {
                    stateManager.updateErrorMessage(result.message)
            ...
            val errorMsg = e.message ?: "Gagal memuat data untuk suami"
            stateManager.updateErrorMessage(errorMsg)
            Result.failure(Exception(errorMsg))
        }
    }

...
fun SuratTugasScreen(
    ...
    val currentStep by remember { derivedStateOf { suratTugasViewModel.currentStep } }
    val showConfirmationDialog by remember { derivedStateOf { suratTugasViewModel.showConfirmationDialog } }
    val showPreviewDialog by remember { derivedStateOf { suratTugasViewModel.showPreviewDialog } }
    val isLoading by remember { derivedStateOf { suratTugasViewModel.isLoading } }
    val hasFormData by remember { derivedStateOf { suratTugasViewModel.hasFormData() } }
    val totalSteps = 2
    ...
    LaunchedEffect(suratTugasViewModel.tugasEvent) {
        suratTugasViewModel.tugasEvent.collect { event ->
            when (event) {
                is SuratTugasViewModel.SuratTugasEvent.SubmitSuccess -> {
                ...
                is SuratTugasViewModel.SuratTugasEvent.SubmitError -> {
                    errorDialogTitle = "Gagal Mengirim"
                    errorDialogMessage = event.message
                    showErrorDialog = true
                }
                is SuratTugasViewModel.SuratTugasEvent.UserDataLoadError -> {
                    errorDialogTitle = "Gagal Memuat Data"
                    errorDialogMessage = event.message
                    showErrorDialog = true
                }
                is SuratTugasViewModel.SuratTugasEvent.ValidationError -> {
                    snackbarHostState.showSnackbar(
                        message = "Mohon lengkapi semua field yang diperlukan",
                        duration = SnackbarDuration.Short
                    )
                }
                is SuratTugasViewModel.SuratTugasEvent.StepChanged -> {
    ...
    Scaffold(
        ...
        bottomBar = {
            AppBottomBar(
                onPreviewClick = {
                    suratTugasViewModel.showPreview()
                },
                onBackClick = if (currentStep > 1) {
                    { suratTugasViewModel.previousStep() }
                } else null,
                onContinueClick = if (currentStep < totalSteps) {
                    { suratTugasViewModel.nextStep() }
                } else null,
                onSubmitClick = if (currentStep == totalSteps) {
                    { suratTugasViewModel.showConfirmationDialog() }
                } else null
            )
        }
    ) { paddingValues ->
        Box(modifier = Modifier.padding(paddingValues)) {
            ...
            if (showPreviewDialog) {
                PreviewDialog(
                    viewModel = suratTugasViewModel,
                    onDismiss = {
                        suratTugasViewModel.dismissPreview()
                    },
                    onSubmit = {
                        suratTugasViewModel.dismissPreview()
                        suratTugasViewModel.showConfirmationDialog()
            ...
            if (showConfirmationDialog) {
                SubmitConfirmationDialog(
                    onConfirm = {
                        suratTugasViewModel.confirmSubmit()
                    },
                    onDismiss = {
                        suratTugasViewModel.dismissConfirmationDialog()
                    },
                    onPreview = {
                        suratTugasViewModel.dismissConfirmationDialog()
                        suratTugasViewModel.showPreview()
            ...
            if (showErrorDialog) {
                ErrorDialog(
                    ...
                    onDismiss = {
                        ...
                        suratTugasViewModel.clearError()
                    }
                )
            }
        ...
        LazyColumn(
            ...
            item {
                PreviewSection(
                    ...
                    content = {
                        PreviewItem("Ditugaskan Untuk", viewModel.ditugaskanUntukValue)
                        PreviewItem("Deskripsi/Detail Tugas", viewModel.deskripsiValue)
                    }
                )
            }

            // Penerima Tugas Utama
            item {
                PreviewSection(
                    title = "Penerima Tugas Utama",
                    content = {
            ...
            // Penerima Tugas Tambahan
            viewModel.additionalRecipients.forEachIndexed { index, recipient ->
                item {
                    PreviewSection(
                        title = "Penerima Tugas ${index + 2}",
                        content = {
                            PreviewItem("NIK", recipient.nik)
                            PreviewItem("Nama Lengkap", recipient.nama)
                            PreviewItem("Jabatan", recipient.jabatan)
                        }
                    )
                }
            }
